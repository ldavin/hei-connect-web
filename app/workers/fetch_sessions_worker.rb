class FetchSessionsWorker
  def schedule(user_id, immediate = false)
    user = User.find user_id

    user.sessions_scheduled!
    perform user_id, immediate
  end

  private

  def perform(user_id, immediatly_schedule_grades_updates)
    user = User.find user_id
    ...
    begin
      ...
      if immediatly_schedule_grades_updates
        user.sessions.sort { |x, y| y.year <=> x.year }.each do |session|
          FetchGradesWorker.new.schedule user.id, session.id
        end
      end
